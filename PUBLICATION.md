...
- **By default nothing else is sent**: the Workshop title, preview image, tags and description are left
  as they are on the page. The workflow generated from the shared template has four opt-in inputs
  (`update_preview`, `update_title`, `update_tags`, `update_description`), off by default; the dry-run
  shows what each would change next to the public page, and only a publish run with the same options
  sends it. Visibility is never sent, and the screenshots below stay a manual step. The workflow is
  generated by `Rimworld-Release-Admin/scripts/generate-publish-workflow.sh`, its values are in
  `.github/publish.config.json`: regenerate it, do not edit the scripts. The description source is
  `Mod/README.template.md` (Markdown, `description.format` `markdown`): with `update_description` on, the CI converts
...
- The description is sent when the item is created, and afterwards only when `update_description` is on for
  that dispatch (source: `Mod/README.template.md`, converted by the CI). Read the converted text in the dry-run
  before approving: it replaces the page's.
...
The description is sent when an item is created, and afterwards only when the `update_description` input is
on (off by default; `dispatch-publish.sh ... --description`, in the dry-run and in the publish). Its source is
`Mod/README.template.md`, in Markdown, kept out of the upload by `Mod/.steamignore`; the CI converts it to
BBCode with `@steamdown/core` (`##` becomes `[h2]`, `**` becomes `[b]`, links become `[url=]`, lists become
`[list]`) and the dry-run prints the converted text. Steam wants BBCode, and `About.xml`'s `<description>` is
not BBCode where it matters: its section titles are plain capitals, not `[h2]`. Keep the section order AUDIT.md requires:
